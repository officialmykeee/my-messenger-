  ...
  <script>
    ...
    // Pre-initialize audio context and attempt to get stream on load
    window.addEventListener('load', () => {
      audioContext = new (window.AudioContext || window.webkitAudioContext)();
      // Attempt to pre-fetch stream (will prompt on first load, handled silently)
      navigator.mediaDevices.getUserMedia({ audio: true })
        ...
        .catch(err => {
          console.log('Pre-fetch failed, will initialize on first touch:', err);
        });
      scrollToLatestMessage();
      inputField.blur();
    });
    ...
    function startDotBlink() {
      ...
      dotBlinkInterval = setInterval(() => {
        if (smileToggler.classList.contains('red-dot') && !smileToggler.classList.contains('trash-icon')) {
          smileToggler.innerHTML = smileToggler.innerHTML === '•' ? '' : '•';
        }
      }, 300);
    }
    ...
    micIcon.addEventListener('touchstart', (e) => {
      ...
      if (emojiPicker.classList.contains('active')) {
        ...
        smileToggler.classList.remove('fa-smile');
        smileToggler.classList.add('red-dot');
        smileTogglerState = 'dot';
      }
      showRecordingInterface();
    ...
    recordingInterface.addEventListener('click', (e) => {
      ...
      if (cancelLink && cancelLink.contains(e.target) && isLocked && isRecording) {
        cancelRecording();
      }
    });
